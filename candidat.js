function onLoad_candidat (){
	...
	Masquer_date_fin_CTT (thisComponent);
	...
	thisComponent.getPDFFileName = function() {
		...
		var complement_contrat = $('#COMPLEMENT_cs00typecontrat').val();
	...
	// JR Le 23/02/2017 ajout limite de caractère pour le numéro de sécurité social et sa clé
	thisComponent.ui.find('[name=t01_23_num_securite_sociale]').attr("maxlength","13");
	// thisComponent.ui.find('[name=t01_23_num_securite_sociale]').removeClass("gopaas-field-number");
	thisComponent.ui.find('[name=t01clesecuritesociale]').attr("maxlength","2");
	...
	function origine_recrutement (thisComponent) { 
		if (thisComponent.getValue ('t01_24_type_recrutement') !== "ORI001") {
		thisComponent.ui.find("#t01_25_nom_cabinet").closest('.form-group').hide();
		thisComponent.ui.find("#cs0autrecabinet").closest('.form-group').hide();
		} else {
		thisComponent.ui.find("#t01_25_nom_cabinet").closest('.form-group').show();	
		thisComponent.ui.find("#cs0autrecabinet").closest('.form-group').show();
		}
	}
	// Origine recrutement
	...
	// AJOUT AKERNECH DU 08/03/2017 champ Date de fin de contrat masqué si type de contrat égal CDI / CDIOD
	
	function Masquer_date_fin_CTT (thisComponent) { 
	
		if (thisComponent.getValue ('t01typecontratGRE') === "CTT001" || thisComponent.getValue ('#COMPLEMENT_t01typecontratGRE') === "CTT006") {
			thisComponent.ui.find("#t01_31_date_fin_contrat").closest('.form-group').hide();	
		}
		
		if (thisComponent.getValue ('t01typecontratGRE') !== "CTT001" && thisComponent.getValue ('#COMPLEMENT_t01typecontratGRE') !== "CTT006") {
			thisComponent.ui.find("#t01_31_date_fin_contrat").closest('.form-group').show();	
		}
	}
	...
	// AJOUT AKERNECH DU 08/03/2017 EVENEMENT SUR LE TYPE DE CONTRAT
	
	
			thisComponent.ui.find("[name=t01typecontratGRE]").data("oldValue", thisComponent.ui.find("[name=t01typecontratGRE]").val());
			var intervalId2 = setInterval(function() {
				if (!thisComponent.ui.closest("body").length) { // si le composant a été supprimé (onglet fermé)
					clearInterval(intervalId2);
					return;
				}
				var t01typecontratGRE = thisComponent.ui.find("[name=t01typecontratGRE]");
				if (t01typecontratGRE.val() !== t01typecontratGRE.data("oldValue")) {
					t01typecontratGRE.data("oldValue", t01typecontratGRE.val());
					Masquer_date_fin_CTT(thisComponent);			
				}
			}, 1000);	
	...
	// AJOUT AKERNECH DU 08/03/2017 EVENEMENT SUR ORIGINE RECRUTEMENT
	
	
			thisComponent.ui.find("[name=t01_24_type_recrutement]").data("oldValue", thisComponent.ui.find("[name=t01_24_type_recrutement]").val());
			var intervalId2 = setInterval(function() {
				if (!thisComponent.ui.closest("body").length) { // si le composant a été supprimé (onglet fermé)
					clearInterval(intervalId2);
					return;
				}
				var t01_24_type_recrutement = thisComponent.ui.find("[name=t01_24_type_recrutement]");
				if (t01_24_type_recrutement.val() !== t01_24_type_recrutement.data("oldValue")) {
					t01_24_type_recrutement.data("oldValue", t01_24_type_recrutement.val());
					origine_recrutement(thisComponent);			
				}
			}, 1000);
	...
	*/
	...
	// JR le 15/02/2017 les find ne fonctionnait pas sur les connexions, correction avec ceux ci-dessous
	...
	this.ui.find("#select_t01_24_type_recrutement").attr("readonly","readonly").attr("disabled","disabled").addClass("disabled");
	...
	if (etape === "Brouillon") {
		...
		if (gsUser !== this.getValue("recruteur") && UTILISATEUR['groupe'] !== "Admin" && UTILISATEUR['groupe'] !== "GRP_ADM_GRE" && UTILISATEUR['groupe'] !== "GRP_RH_CORP") {
			...
			this.ui.find("#select_t01_03_civilite_candidat").removeAttr("readonly","readonly").removeAttr("disabled","disabled").removeClass("disabled");
			...
			// pour les connexions liste il faut retirer les attributs sur le #select
			this.ui.find("#select_t01_16_statut").removeAttr("readonly","readonly").removeAttr("disabled","disabled").removeClass("disabled");
			this.ui.find("#select_t01typecontratGRE").removeAttr("readonly","readonly").removeAttr("disabled","disabled").removeClass("disabled");
		
			
			
			// Onglet Developpement RH > Informations Développement RH
			this.ui.find("#select_t01_25_nom_cabinet").removeAttr("readonly","readonly").removeAttr("disabled","disabled").removeClass("disabled");
			this.ui.find("#select_t01_24_type_recrutement").removeAttr("readonly","readonly").removeAttr("disabled","disabled").removeClass("disabled");
			...
			addButton("Soumettre le candidat", "Soumettre le candidat", function() {
				...
				$.get("template_auto/candidat/candidat.php", {
					...
					if (thisComponent.getValue('valideur1') == '' ) {
						...
						gopaas.dialog.info("Votre fiche candidat sélectionné présente une anomalie. Merci de bien vouloir enregistrer en brouillon votre candidat et tenter de nouveau l'envoi. Si le problème persiste, contacter le support à l’adresse : adm-candidat-selectionne@altareacogedim.com");
	...
	// Attente valideur
		...
		// si c'est le valideur en cours qui a ouvert la fiche :
			...
			if (gsUser === this.getValue("recruteur") || UTILISATEUR['groupe'] === "Admin" || UTILISATEUR['groupe'] === "GRP_ADM_GRE" || UTILISATEUR['groupe'] === "GRP_RH_CORP"){
				// si c'est le RECRUTEUR ou un ADMIN qui a ouvert la fiche on Active les champs			
				// Onglet PRINCIPAL > Informations Contractuelles			
				this.ui.find("#COMPLEMENT_t01_03_civilite_candidat").removeAttr("readonly","readonly").removeAttr("disabled","disabled").removeClass("disabled");
				this.ui.find("#COMPLEMENT_t01_16_statut").removeAttr("readonly","readonly").removeAttr("disabled","disabled").removeClass("disabled");
				this.ui.find("#combo_t01_19_nb_jours_rtt_monetises").removeAttr("readonly","readonly").removeAttr("disabled","disabled").removeClass("disabled");

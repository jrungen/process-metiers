...
// window.console.log = window.console.log || function(message) {};
...
function onLoad_demande () {
	...
	//-----------------------------------------------
	// thisComponent.afficherMasquerService = function() {
		// console.log('entré dans afficherMasquerService');
		// var direction = this.getValue("d00direction"),
			// hasService = false
		// ;
		// if (!this.listeService) {
			// return false; // la liste des services n'est pas encore chargée ! La valeur de retour 'false' indique qu'il faudra réessayer plus tard
		// }

		// for (var i=0; i<this.listeService.length; i++) {
			// var service = this.listeService[i];
		...
		// }
		// if (hasService) {
			// console.log('Affichage sous_service');
			// this.ui.find("[name=sous_service]").closest(".form-group").show();
		// } else {
			// console.log('masquer sous_service');
			// this.ui.find("[name=sous_service]").closest(".form-group").hide();
		// }
		// return true;
	// };
	...
	thisComponent.getPDFFileName = function() {
		var data = [
			...
			($('#COMPLEMENT_r06typecontrat').val() || '').replace(/[^\w ]/g,''),
	...
	function afficher_cdi(thisComponent){
		...
		// JR 07/02/2017 MAJ connexions motif_recrutement et valeur retourné MOTE002 = Remplacement
		// JR 10/02/2017 correction  condition motif_recrutement - j'ai supprimé les LOWERCASE les valeurs sont en majuscule.
		var motif_recrutement = thisComponent.getValue("d00personneremplacee");
		if (motif_recrutement === 'MOTE002') {
	...
	//-----------------------------------------------
	// Société
	// JR le 10/02/2017 Sera inutile une fois les filtes sur connexion actif	
	// var onLoadMetier = thisComponent.ui.find("[name=d00metiers]").val();
	// thisComponent.ui.find("#combo_societe").on("focusout", function(){
		// var societe = $(this).val();
		// console.log(thisComponent.ui.find("#combo_metier").val());
		// // recréer la liste métier
		// // Vider puis créer
		// // #combo_metier
		// // •	Si société = Cogedim Gestion, Cogedim Vente ; métier = Bureau ou logement ou support
		// // •	Si société = CSE ; métier = CSE - Exploitation
		// // •	Si société = Altarea France ou Altarea Management ; métier = commerce ou support
		// // Bureau, Logement,Commerce,CSE - Exploitation,Support
		
		// if ((societe === 'Cogedim Gestion')||(societe === 'Cogedim Vente')) {
			// thisComponent.ui.find("#combo_metier").empty(); // vide
			// thisComponent.ui.find("#combo_metier").append($('<option></option>').attr("value", 'Bureau').text('Bureau'));
			// thisComponent.ui.find("#combo_metier").append($('<option></option>').attr("value", 'Logement').text('Logement'));
			// thisComponent.ui.find("#combo_metier").append($('<option></option>').attr("value", 'Support').text('Support'));
			
		// }else if (societe === 'CSE') {
			// thisComponent.ui.find("#combo_metier").empty(); // vide
			// thisComponent.ui.find("#combo_metier").append($('<option></option>').attr("value", 'CSE - Exploitation').text('CSE - Exploitation'));
			
	    // }else if (societe === 'Altarea France') {
			// thisComponent.ui.find("#combo_metier").empty(); // vide
			// thisComponent.ui.find("#combo_metier").append($('<option></option>').attr("value", 'Commerce').text('Commerce'));
			// thisComponent.ui.find("#combo_metier").append($('<option></option>').attr("value", 'Support').text('Support'));
		// }else if (societe === 'Altarea Management') {
			 // thisComponent.ui.find("#combo_metier").empty(); // vide
			 // thisComponent.ui.find("#combo_metier").append($('<option></option>').attr("value", 'Support').text('Support'));	
		// }else{
			// thisComponent.ui.find("#combo_metier").empty(); // vide
	...
	// }).trigger("focusout"); 
	// thisComponent.ui.find("#combo_metier").val(onLoadMetier);
	
	//-----------------------------------------------
	// Intitulé poste
	// JR le 15/02/2017 inutile car remplacé par une connexion
	// thisComponent.ui.find("#intitule_poste").on("blur", function() {
	...
	/********************************************************************************/
	// JR LE 10/02/2017 NOUVEAU SCRIPT POUR CHAMP MOTIF RECRUTEMENT et TYPE CONTRAT //
	/********************************************************************************/
	// lancer la vérification au chargement
	...
	//-----------------------------------------------
	// Direction
	// // JR 07/02/2017 correction du script pour prendre en compte les nouvelles listes connexion
	// thisComponent.ui.find("[name=d00direction]").closest(".input-group").find(".input-group-btn button:first").removeAttr("onclick").off("click").click(function() { // removeAttr("onclick") + off("click") désactive le click s'il a été assigné par JQuery ET s'il a été assigné directement en HTML. http://stackoverflow.com/questions/785147/jquery-unbind-click
		// if (!thisComponent.getValue("d00societes")) {
			...
			// return;
		// }
		// // JR le 10/02/2017 Dès que le filtre sur les connexions liste fonctionnera il faudra corriger la ligne suivante
		// gopaas.util.connectionAsList($('#'+this.form.id +' #select_d00direction'),'r15direction');$('#select_d00direction').css('display','block');
	// });
	...
	// on ne peut pas utiliser les évènements sur un champ de type connexion,
	// il faut faire un contrôle toutes les x secondes pour détecter les modifications faites par JavaScript.
	// JR le 10/02/2017 Correction de la connexion direction
	// JR le 14/02/2017 je met en commentaire car on n'utilise plus les sous_service
	// thisComponent.ui.find("[name=d00direction]").data("oldValue", thisComponent.ui.find("[name=d00direction]").val());
	// var intervalId = setInterval(function() {
		// if (!thisComponent.ui.closest("body").length) { // si le composant a été supprimé (onglet fermé)
			// clearInterval(intervalId);
			// return;
		// }
		// var direction = thisComponent.ui.find("[name=d00direction]");
		// if (direction.val() !== direction.data("oldValue")) {
			// if (thisComponent.afficherMasquerService()) { // cette fonction renvoie false si la liste des services n'est pas encore chargée, auquel cas il faut réessayer plus tard
				// direction.data("oldValue", direction.val());
			// }
		// }
	// }, 1000);
	...
	// Arbitrage
		...
		if (gsUser !== mpl && (UTILISATEUR['groupe'].toLowerCase() !== "admin") && (UTILISATEUR['groupe'].toLowerCase() !== "grp_adm_dar")) {
			this.ui.find("input, select, textarea").attr("readonly","readonly").attr("disabled","disabled").addClass("disabled"); // désactive tous les input (ne fonctionne pas totalement pour les listes déroulante)
			this.ui.find(".gopaas-button-save-and-close, .gopaas-button-save").hide(); // cache les 2 boutons d'enregistrement
...
function calculImpactBudgetaire(thisComponent){
	...
	// budget
	impact_budgetaire_annee_n = Number(salaire) * (Math.max(Number(diff_annee_n.day)+1,0)/365) * 1.47; // +1 car date_diff ne comptabilise pas le dernier jour
	impact_budgetaire_annee_n_plus_1 = Number(salaire) * (Math.max(Number(diff_annee_n_plus_1.day+1),0)/365) * 1.47; // +1 car date_diff ne comptabilise pas le dernier jour
	impact_budgetaire_annee_pleine = Number(salaire) * 1.47;
